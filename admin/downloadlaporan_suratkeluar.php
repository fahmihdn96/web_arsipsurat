...
// Set properties
$excelku->getProperties()->setCreator("Fahmi")
...
$excelku->getActiveSheet()->setCellValue('A2', "PEMERINTAH KABUPATEN BOGOR");
$excelku->getActiveSheet()->mergeCells('A3:G3');
$excelku->getActiveSheet()->setCellValue('A3', "KECAMATAN CIOMAS");
$excelku->getActiveSheet()->mergeCells('A4:G4');
$excelku->getActiveSheet()->setCellValue('A4', "KANTOR DESA PARAKAN");
$excelku->getActiveSheet()->mergeCells('A5:G5');
$excelku->getActiveSheet()->setCellValue('A5', "Jl. H. Abdullah  No. 01 Parakan Ciomas 16616 Bogor");
$excelku->getActiveSheet()->mergeCells('A6:G6');
$excelku->getActiveSheet()->setCellValue('A6', "DATA SURAT KELUAR BULAN $bulan TAHUN $tahun");
$excelku->getActiveSheet()->getStyle('A2:G6')->getFont()->setName('Arial');
$excelku->getActiveSheet()->getStyle('A2:G6')->getFont()->setSize(12);
$excelku->getActiveSheet()->getStyle('A2:G6')->getFont()->setBold(true);
$excelku->getActiveSheet()->getStyle('A2:G6')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$excelku->getActiveSheet()->getStyle('A8:G8')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$excelku->getActiveSheet()->getStyle('A8:G8')->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
...
$SI = $excelku->setActiveSheetIndex(0);
 ...
 $SI->setCellValue('D8', "NAMA BAGIAN");
 $SI->setCellValue('E8', "TANGAL SURAT");
 $SI->setCellValue('F8', "KEPADA");
 $SI->setCellValue('G8', "PERIHAL");
...
while ($data = $query1->fetch_assoc()) {
  ...
  $SI->setCellValue("D".$baris,$data['nama_bagian']); 
  $SI->setCellValue("E".$baris,$data['tanggalsurat_suratkeluar']); 
  $SI->setCellValue("F".$baris,$data['kepada_suratkeluar']); 
  $SI->setCellValue("G".$baris,$data['perihal_suratkeluar']); 
  ...
  // Set lebar kolom
    ...
    $excelku->getActiveSheet()->getStyle('A8:H'.$baris.'')->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
    $excelku->getActiveSheet()->getStyle('G8:H'.$baris.'')->getAlignment()->setWrapText(true);
    //wraptext
}
